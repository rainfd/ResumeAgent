...
### 3. AI Agent 集成模块
- [ ] 3.1. 创建 LLM Agent 抽象基类
  - 创建 `core/agents/base.py` 定义 LLMAgent 抽象接口
  - 实现分析、优化建议、打招呼语生成的抽象方法
  - 创建 AgentFactory 工厂类
  - **需求参考**: 可扩展性设计，支持多种LLM

- [ ] 3.2. 实现 DeepSeek Agent
  - 创建 `core/agents/deepseek.py` DeepSeek具体实现
  - 实现异步 HTTP 客户端和 API 调用
  - 添加 API 密钥管理和请求认证
...
- [ ] 6.2. 实现 Markdown 简历解析功能
  - 使用 python-markdown 解析 Markdown 格式
  - 实现统一的内容提取接口
  - 保持原始文本完整性用于后续对比
